...
const CARRIER_MIN_VERSION: u32 = 1;
// Value returned on success. We use libc's errors otherwise.
const CARRIER_SUCCESS: i32 = 0;
...
#[link(name = "carriercore")]
extern "C" {
    #[cfg(feature = "amd-sev")]
    fn carrier_get_qboot(size: *mut size_t) -> *mut c_char;
    #[cfg(feature = "amd-sev")]
    fn carrier_get_initrd(size: *mut size_t) -> *mut c_char;
    fn carrier_get_kernel(load_addr: *mut u64, size: *mut size_t) -> *mut c_char;
    fn carrier_get_version() -> u32;
}

#[no_mangle]
pub extern "C" fn carrier_set_log_level(level: u32) -> i32 {
    ...
    if LOGGER
        ...
        .configure(Some(format!("lib-{}", process::id())))
        .is_err()
    {
        return -libc::EINVAL;
    }
...
#[no_mangle]
pub extern "C" fn carrier_create_ctx() -> i32 {
    let carrier_version = unsafe { carrier_get_version() };
    if carrier_version < CARRIER_MIN_VERSION {
        warn!("Unsupported carrier core version: {}", carrier_version);
        return -libc::EINVAL;
    }
    ...
    #[cfg(feature = "amd-sev")]
    {
        ...
        let qboot_host_addr = unsafe { carrier_get_qboot(&mut qboot_size as *mut usize) };
        ...
        let initrd_host_addr = unsafe { carrier_get_initrd(&mut initrd_size as *mut usize) };
    ...
    if ctx_id == i32::MAX || CTX_MAP.lock().unwrap().contains_key(&(ctx_id as u32)) {
        // lib is not intended to be used as a daemon for managing VMs.
...
#[no_mangle]
pub extern "C" fn carrier_free_ctx(ctx_id: u32) -> i32 {
    match CTX_MAP.lock().unwrap().remove(&ctx_id) {
        Some(_) => CARRIER_SUCCESS,
        None => -libc::ENOENT,
    }
}

#[no_mangle]
pub extern "C" fn carrier_set_vm_config(ctx_id: u32, num_vcpus: u32, ram_mib: u32) -> i32 {
...
#[no_mangle]
#[cfg(not(feature = "amd-sev"))]
pub unsafe extern "C" fn carrier_set_root(ctx_id: u32, c_root_path: *const c_char) -> i32 {
...
#[no_mangle]
#[cfg(not(feature = "amd-sev"))]
pub unsafe extern "C" fn carrier_set_mapped_volumes(
    ctx_id: u32,
...
#[no_mangle]
#[cfg(feature = "amd-sev")]
pub unsafe extern "C" fn carrier_set_root_disk(ctx_id: u32, c_disk_path: *const c_char) -> i32 {
...
#[allow(clippy::missing_safety_doc)]
#[no_mangle]
pub unsafe extern "C" fn carrier_set_port_map(ctx_id: u32, c_port_map: *const *const c_char) -> i32 {
...
#[allow(clippy::missing_safety_doc)]
#[no_mangle]
pub unsafe extern "C" fn carrier_set_rlimits(ctx_id: u32, c_rlimits: *const *const c_char) -> i32 {
...
#[allow(clippy::missing_safety_doc)]
#[no_mangle]
pub unsafe extern "C" fn carrier_set_workdir(ctx_id: u32, c_workdir_path: *const c_char) -> i32 {
...
#[allow(clippy::missing_safety_doc)]
#[no_mangle]
pub unsafe extern "C" fn carrier_set_exec(
    ctx_id: u32,
...
#[no_mangle]
#[cfg(feature = "amd-sev")]
pub unsafe extern "C" fn carrier_set_attestation_url(ctx_id: u32, c_url: *const c_char) -> i32 {
...
#[no_mangle]
pub extern "C" fn carrier_start_enter(ctx_id: u32) -> i32 {
    #[cfg(target_os = "linux")]
    {
        let prname = match env::var("HOSTNAME") {
            Ok(val) => CString::new(format!("VM:{}", val)).unwrap(),
            Err(_) => CString::new("lib VM").unwrap(),
        };
